    ...
    return {
        ...
        controller: ["$scope","$attrs","$element", function($scope, $attrs, $element) {
            ...
            tagsInputConfig.load($scope, $attrs, {
                ...
                allowedTagsPattern: { type: RegExp, defaultValue: /^[a-zA-ñ-Z0-9\s]+$/ },
...
tagsInput.directive('autoComplete', ["$document","$timeout","$sce","tagsInputConfig", function($document, $timeout, $sce, tagsInputConfig) {
    function SuggestionList(loadFn, options) {
        ...
        /*modified by saul because the respond of promise is an object and not a array string*/
        getDifference = function(respondPromise, CurrentTags) {
            var result = [];
            var arrayTags = respondPromise.data;
            arrayTags.forEach(function(item) {
                if (CurrentTags.indexOf(item) === -1) {
                    result.push(item);
                }
            });
            return result;
